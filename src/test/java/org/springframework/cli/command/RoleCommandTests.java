...
public class RoleCommandTests {
	...
	void defaultRole(final @TempDir Path workingDir) {
		this.contextRunner.withUserConfiguration(MockUserConfig.class).run(context -> {
			...
			Path varsFilePath = workingDir.resolve(".spring").resolve("roles").resolve("vars").resolve("vars.yml");
	...
	void qaRole(final @TempDir Path workingDir) {
		this.contextRunner.withUserConfiguration(MockUserConfig.class).run(context -> {
			...
			Path varsFilePath = workingDir.resolve(".spring").resolve("roles").resolve("vars").resolve("vars-qa.yml");
